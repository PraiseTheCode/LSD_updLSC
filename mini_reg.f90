...
subroutine mini_reg(xopt,absc,obs,weight,regpar,n,nwl,fun) ! Tichonov's regularization included
...
!  Compute regularization and total function value
  call regfunq(x,absc,aderiv,regpar,n,regul)
  fvalue=d+regul 
...
subroutine regfunq(x,absc,aderiv,regpar,n,regul)
...
!  Tikhonov regularization, quadratic form
 regul=0.d0
  do i=2,n
   ...
   da=absc(i1)-absc(i2)
   df=x(i1)-x(i2)
   regul=regul+wt*(df/da)**2
  enddo 

! Scale by regularization parameter 
  regul=regpar(1)*regul/n  
